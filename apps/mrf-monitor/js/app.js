import { loadAllRecords, loadApplicantRecords, saveRecord } from './storage.js';
import { toast, compressImage } from './utils.js';
import { records, setRecords, setTotalCandidates, renderDashboard, renderTable, openModal, closeModal, saveModalChanges, deleteCurrentModalRecord } from './ui.js';
...
if (form) {
  form.addEventListener('submit', async e => {
    ...
    const optimisticRecord = { ...rec, isOptimistic: true };
    const optimisticList = [optimisticRecord, ...records];
    setRecords(optimisticList);
    renderDashboard();
    renderTable();
    window.HR_PORTAL_SYNC?.beginOptimisticUpdate?.('mrf-dashboard', rec.id, optimisticRecord, 'Saving request...');
    toast('Request saved locally — syncing...');
    ...
    try {
      ...
      renderDashboard();
      renderTable();
      window.HR_PORTAL_SYNC?.resolveOptimisticUpdate?.('mrf-dashboard', 'Request synced successfully.');
      ...
      const failedList = records.filter(item => item.id !== optimisticRecord.id);
      failedList.unshift({ ...optimisticRecord, isPending: true, syncFailed: true });
      setRecords(failedList);
      renderDashboard();
      renderTable();
      window.HR_PORTAL_SYNC?.rejectOptimisticUpdate?.('mrf-dashboard', err.message || 'Save failed — try again.');
...
async function refreshFromServer({ silent = false } = {}) {
  ...
  try {
    const [loaded, applicants] = await Promise.all([loadAllRecords(), loadApplicantRecords()]);
    setRecords(loaded);
    setTotalCandidates(applicants.length);
    renderTable();
    renderDashboard();
