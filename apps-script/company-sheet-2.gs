...
const MRF_FIELDS = [
  ...
  ['status', 'Status', ['status']], ['remarks', 'Remarks', ['remarks', 'notes']],
...
function saveMrf(input) {
  ...
  try {
    ...
    const record = { id: id, mrfNumber: cleanUpper(input.mrfNumber), department: clean(input.department), position: clean(input.position), originalHeadcount: baseHeadcount, assignedHeadcount: 0, dateRequested: clean(input.dateRequested), dateNeeded: clean(input.dateNeeded), requestedBy: clean(input.requestedBy), status: normalizeMrfStatus(input.status), remarks: clean(input.remarks), fileUrl: fileUrl, createdAt: normalizeDateTime(input.createdAt || now), updatedAt: normalizeDateTime(now) };
...
function deleteById(name, fields, id) { const sheet = getSheet(name, fields); const map = ensureSchema(sheet, fields); const row = findRow(sheet, map.id, id); if (row) sheet.deleteRow(row); if (name === CONFIG.mrfSheet) syncMrfHeadcounts(); }

function getApplicantOptions() {
  syncMrfHeadcounts();
  applyDropdownValidations();
  clearApplicantDropdowns();
  return { ok: true, mrfNumbers: getMrfOptions() };
}
...
function setupDatabase() {
  ...
  ensureSchema(mrfSheet, MRF_FIELDS);
  ensureSchema(applicantSheet, APPLICANT_FIELDS);
  clearApplicantRowBackgrounds(applicantSheet);
  syncMrfHeadcounts();
  applyDropdownValidations();
  clearApplicantDropdowns();
}
...
function getMrfOptions(includeAvailableOnly = false) {
  ...
  const options = rows
    ...
    .filter(mrfNumber => {
      ...
      return original > 0 && assigned < original;
    });
...
function applyDropdownValidations() {
  ...
  const applicantSheet = spreadsheet.getSheetByName(CONFIG.applicantsSheet) || CONFIG.applicantSheetAliases.map(name => spreadsheet.getSheetByName(name)).find(Boolean);
  const ruleOptions = { showDropdown: true, showCustomUi: true, allowInvalid: false };

  if (mrfSheet) {
    const mrfMap = ensureSchema(mrfSheet, MRF_FIELDS);
    const statusRule = SpreadsheetApp.newDataValidation().requireValueInList(MRF_STATUSES, true).setAllowInvalid(ruleOptions.allowInvalid).build();
    mrfSheet.getRange(2, mrfMap.status, Math.max(mrfSheet.getMaxRows() - 1, 1), 1).setDataValidation(statusRule);
  }

  if (applicantSheet) {
    const applicantMap = ensureSchema(applicantSheet, APPLICANT_FIELDS);
    const genderRule = SpreadsheetApp.newDataValidation().requireValueInList(GENDER_OPTIONS, true).setAllowInvalid(ruleOptions.allowInvalid).build();
    applicantSheet.getRange(2, applicantMap.gender, Math.max(applicantSheet.getMaxRows() - 1, 1), 1).setDataValidation(genderRule);
  }
}

function clearApplicantRowBackgrounds(sheet) {
  if (!sheet || sheet.getMaxRows() < 2 || sheet.getMaxColumns() < APPLICANT_FIELDS.length) return;
  sheet.getRange(2, 1, sheet.getMaxRows() - 1, APPLICANT_FIELDS.length).setBackground(null);
}
...
function formatSheet(sheet, count) { sheet.getRange(1, 1, 1, count).setFontFamily('Arial').setFontSize(9).setFontWeight('bold').setHorizontalAlignment('center'); if (sheet.getMaxRows() > 1) sheet.getRange(2, 1, sheet.getMaxRows() - 1, count).setFontFamily('Arial').setFontSize(9); const headers = sheet.getRange(1, 1, 1, count).getValues()[0].map(normalizeHeader); ['createdat', 'updatedat'].forEach(header => { const column = headers.indexOf(header) + 1; if (column > 0 && sheet.getMaxRows() > 1) sheet.getRange(2, column, sheet.getMaxRows() - 1, 1).setNumberFormat('mm/dd/yyyy hh:mm AM/PM'); }); normalizeIdColumn(sheet); sheet.setFrozenRows(1); }
...
function normalizeDateTime(value) { if (value instanceof Date) return value; if (value == null || value === '') return ''; const number = Number(value); if (Number.isFinite(number)) { if (number > 100000000000) return new Date(number); if (number > 20000 && number < 100000) return new Date(Math.round((number - 25569) * 86400000)); } const parsed = new Date(value); return Number.isNaN(parsed.getTime()) ? value : parsed; }
